...
# Analysis mode
ANALYSIS_MODE = "single"  # "batch" = analyze all 15, "single" = analyze one scenario
SELECTED_SCENARIO = "20251120_110130_de_lu_crate0.5"  # Used only if ANALYSIS_MODE = "single"
...
for i, scenario_dir in enumerate(scenarios_to_analyze, 1):
    ...
    if GENERATE_PLOTS:
        ...
        print("  [4b/10] Combined Market Overview (4-in-1)...")
        fig_combined = make_subplots(
            rows=4, cols=1,
            subplot_titles=(
                f"Day-Ahead Market - {title_suffix}",
                f"aFRR Energy Market - {title_suffix}",
                f"Capacity Markets - {title_suffix}",
                f"Battery Schedule: SOC & Power Bids - {title_suffix}"
            ),
        ...
        for trace in fig1.data:
            fig_combined.add_trace(trace, row=1, col=1, secondary_y=("Price" in trace.name or "price" in trace.name.lower()))

        for trace in fig2.data:
            fig_combined.add_trace(trace, row=2, col=1, secondary_y=("Price" in trace.name or "price" in trace.name.lower()))

        for trace in fig3.data:
            fig_combined.add_trace(trace, row=3, col=1, secondary_y=("Price" in trace.name or "price" in trace.name.lower()))

        for trace in fig4.data:
            fig_combined.add_trace(trace, row=4, col=1, secondary_y=("SOC" in trace.name))

        # Update layout
        fig_combined.update_layout(
            height=2000,  # 500px per subplot
            showlegend=True,
            title_text=f"Complete Market Overview - {title_suffix}",
            title_font_size=16,
            font=dict(family='Arial', size=10)
        )

        # Update x-axes
        fig_combined.update_xaxes(title_text="Timestamp", row=4, col=1)

        if SAVE_PLOTS:
            fig_combined.write_html(str(plots_dir / f"combined_market_overview.{PLOT_FORMAT}"))
