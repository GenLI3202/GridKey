...
#### Configuration Files
- **Unified config**: `config/Config.yml` - Single YAML file containing all configuration sections:
  - `solver_config` - Solver settings (timeouts, tolerances, solver options)
  - `aging_config` - Degradation model parameters (cyclic + calendar aging)
  - `afrr_ev_weights_config` - aFRR activation probabilities (historical_activation section)
  - `investment_config` - Investment/financial parameters
  - `mpc_config` - MPC controller settings (horizon, execution hours)
  - `mpc_test_config` - MPC test scenario configuration

- **Config Loader**: Use `src/utils/config_loader.py` to access configuration:
  ```python
...
**Solver Settings** (configured in `config/Config.yml` → `solver_config` section):
...
- 6 segments for cyclic aging (750 kWh each with 4500 kWh capacity)
- Piecewise linear cost function: 0.0087-0.0955 EUR/kWh
...
- `HORIZON_HOURS` / `EXECUTION_HOURS`: from `config/Config.yml` → `mpc_config`
- `DEFAULT_SOLVER`: from `config/Config.yml` → `solver_config` (currently `cplex`)
